...
@Component({
  selector: 'app-alt-data-entry',
  templateUrl: './alt-data-entry.component.html',
  styleUrls: ['./alt-data-entry.component.scss'],
})
export class AltDataEntryComponent implements OnInit, AfterViewInit {
  ...
  generateSubmission(): Submission {
    ...
    if (!this.autoFill) {
      ...
      let matchingDepartures = this.departures.filter(
        (c) =>
      ...
      if (matchingDepartures.length === 1) {
        ...
        match = matchingDepartures.sort(
          (a, b) => a.departure_time.valueOf() - b.departure_time.valueOf()
        )[0];
      }
    } else {
  ...
  refreshStopNames() {
    ...
    this.stops = this.departures
      .filter(
        (dep) =>
          dep.route_type === this.route_type &&
          dep.route_id === this.selected_route.route_id &&
          dep.direction_id === this.selected_direction.direction_id
      ) //filter by selected route type
      ...
      .filter(
        (thing, i, arr) =>
          arr.findIndex((t) => t.stop_id === thing.stop_id) === i
      ); //get distinct values
    ...
    if (this.selected_route !== undefined) {
      this.stopsService
        .getStopsByRoute(
          ...
          this.getRouteTypeNumber(this.selected_route.route_type)
        )
          ...
          response.subscribe((c) => {
            ...
            if (this.geoService.isGeoLocationEnabled) {
              ...
              this.geoService.getPosition().then((pos) => {
                oStops.forEach(
                  (stp) =>
                    (stp.stop_distance = this.getEuclideanDistance(
                      [stp.stop_latitude, stp.stop_longitude],
                      [pos.lat, pos.lng]
                    ))
                );
              });
            ...
            this.otherStops = oStops
              .filter(
                (stp) => !this.stops.map((c) => c.stop_id).includes(stp.stop_id)
              )
  ...
  refreshRouteNames() {
    ...
    this.routes = this.departures
      ...
      .filter(
        (thing, i, arr) =>
          arr.findIndex((t) => t.route_id === thing.route_id) == i
    ...
    this.directions = this.departures
      .filter(
        (dep) =>
          dep.route_type === this.route_type &&
          dep.route_id === this.selected_route.route_id
      ) //filter by selected route type and stop name
      .map((c) => {
        return {
      ...
      .filter(
        (thing, i, arr) =>
          arr.findIndex((t) => t.direction_id === thing.direction_id) == i
      );
